import React from "react";
import {
  Button,
  ModalHeader,
  ModalBody,
  ModalFooter,
  Input,
  Card,
  CardBody,
  Label,
  Row,
  CustomInput,
} from "reactstrap";
import Select from "react-select";
import CustomSelectInput from "../../../components/common/CustomSelectInput";
import IntlMessages from "../../../helpers/IntlMessages";
import { servicePath } from "../../../constants/defaultValues";
import { NotificationManager } from "../../../components/common/react-notifications";
import { Colxx } from "../../../components/common/CustomBootstrap";
import axios from "axios";
import { trackPromise } from "react-promise-tracker";

class EditSetting extends React.Component {
  constructor(props) {
    super(props);

    this.state = {
      shop_Obj: [],
      cities: [],
      all_cities_json: [],
      city_areas_json: [],
    };
  }

  async componentDidMount() {
    const { match } = this.props;
    let shop_Obj = await this.get_shop(match.params.id);
    let all_cities = JSON.parse(localStorage.getItem("cities"));
    let cities_dropdown_data = this.cities_dropdown_json(all_cities);

    this.setState({
      shop_Obj: shop_Obj,
      all_cities: all_cities,
      cities_dropdown_data: cities_dropdown_data,
    });
  }
  cities_dropdown_json = (all_cities) => {
    let cities_dropdown_data = [];
    if (all_cities !== undefined && all_cities.length) {
      all_cities.forEach(function (currentValue) {
        cities_dropdown_data.push({
          label: currentValue.city_name,
          value: currentValue.id,
          key: currentValue.id,
        });
      });
    }
    return cities_dropdown_data;
  };

  update_shop = async (event) => {
    event.preventDefault();
    let formData = new FormData(event.target);

    await trackPromise(
      axios({
        method: "put",
        url:
          servicePath +
          "/api/inventory/shops/" +
          this.state.shop_Obj.id +
          ".json",
        data: formData,
        headers: {
          "Content-Type": "application/json",
          "AUTH-TOKEN": localStorage.getItem("auth_token"),
        },
      })
    )
      .then((response) => {
        if (response.status === 200) {
          NotificationManager.success(
            "Shop Updated Successfully",
            "",
            3000,
            null,
            null,
            "filled"
          );
          this.props.history.push("/app/shops/list");
        } else {
          console.log(response);
        }
      })
      .catch((error) => {
        NotificationManager.error(
          error.response.data.message,
          "",
          5000,
          () => {
            alert("callback");
          },
          null,
          "filled"
        );
        console.log("error", error);
      });
  };

  get_shop = async (shop_id) => {
    let shop_Obj = null;

    await trackPromise(
      axios({
        method: "get",
        url: servicePath + "/api/inventory/shops/" + shop_id + ".json",
        headers: {
          "Content-Type": "multipart/form-data",
          "AUTH-TOKEN": localStorage.getItem("auth_token"),
          "IS-ACCESSIBLE": true,
        },
      })
    )
      .then((response) => {
        if (response.status === 200) {
          shop_Obj = response.data;
        } else {
          console.log(response);
        }
      })
      .catch((error) => {
        NotificationManager.error(
          error.response.data.message,
          "",
          5000,
          () => {
            alert("callback");
          },
          null,
          "filled"
        );
        console.log("error", error);
      });
    return shop_Obj;
  };

  render() {
    const { shop_Obj, cities_dropdown_data } = this.state;
    let self = this;
    return (
      <>
        {shop_Obj !== undefined ? (
          <>
            <Colxx xxs="12" lg="6" className="m-auto">
              <Card>
                <CardBody>
                  <form onSubmit={this.update_shop}>
                    <ModalHeader>Edit Shop</ModalHeader>

                    <ModalBody>
                      <Label className="mt-4">Name *</Label>
                      <Input
                        className="form-control"
                        name="shop[shop_name]"
                        type="text"
                        defaultValue={shop_Obj.shop_name}
                        required
                      />
                      <CustomInput
                        className="mt-2"
                        type="checkbox"
                        label="Warehouse"
                        defaultChecked={shop_Obj.is_warehouse}
                        onChange={(event) => self.handleCheckChange(event)}
                      />
                      <Input
                        hidden
                        name="shop[is_warehouse]"
                        defaultValue={shop_Obj.is_warehouse}
                      />
                      <Label className="mt-4">Common Percentage *</Label>
                      <Input
                        className="form-control"
                        name="shop[comm_percentage]"
                        type="number"
                        defaultValue={shop_Obj.comm_percentage}
                        required
                      />
                      <Label className="mt-4">Phone *</Label>
                      <Input
                        className="form-control"
                        name="shop[phone]"
                        type="text"
                        defaultValue={shop_Obj.phone}
                        required
                      />
                      <Label className="mt-4">Address *</Label>
                      <Input
                        className="form-control"
                        name="shop[shop_address]"
                        type="text"
                        defaultValue={shop_Obj.shop_address}
                        required
                      />
                      <Label className="mt-2">Select City</Label>
                      <Row>
                        <Colxx xxs="12" md="12">
                          <Select
                            components={{ Input: CustomSelectInput }}
                            name="shop[city_id]"
                            className="react-select"
                            classNamePrefix="react-select"
                            // onChange={self.handle_product_category_change}
                            options={cities_dropdown_data}
                            required
                          />
                        </Colxx>
                      </Row>
                    </ModalBody>

                    {/* {this.render_address_details()} */}

                    <ModalFooter>
                      <Button
                        color="secondary"
                        outline
                        type="button"
                        onClick={this.props.history.goBack}
                      >
                        <IntlMessages id="pages.cancel" />
                      </Button>
                      <Button color="primary" type="submit">
                        <IntlMessages id="pages.submit" />
                      </Button>
                    </ModalFooter>
                  </form>
                </CardBody>
              </Card>
            </Colxx>
          </>
        ) : (
          <></>
        )}
      </>
    );
  }
}

export default EditSetting;
